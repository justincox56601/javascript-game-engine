 ...
 export class GameEngine{
	...
	constructor(canvas){
		...
		window.addEventListener('resize', e=>{
			//TO DO:  add code to automatically resize canvas and viewport on window change
	...
	_getEntity(name){
		...
		Object.keys(this._entities).forEach(key=>{
			if(String(this._entities[key].name) == String(name)){
				result =  this._entities[key];
			}
		});
		return result;
	}
	...
	_gameLoop(){
		...
		const camera = this._getEntity('Camera');		
		...
		//Update
		const cView = camera.getCurrentView();
		const lyst = [];
		...
		this._canvas.ctx.rect(0, 0, camera.viewPort.x, camera.viewPort.y);
...
export class Camera extends Entity{
	...
	getTransformMatrix(){
		const center = this.viewPort.scalar(0.5);						//center of the view port
		const view = this.transform.position.subtract(this.baseResolution.scalar(0.5));		//where pos is in realtion to the top left corner of bast resolution window
		const screen = center.subtract(this.viewPort.scalar(0.5)); 		//Uppler left corner of the view port window
		const tVector = screen.subtract(view).scalar(this.scale);		//vector pointing from the view upper left to the screen upper left scaled
		let matrix = Matrix.identityMatrix(5,5);						//transformation matrix to hold transform from world space to screen space
	...
	getCurrentView(){
		...
		//array [x1,y1,x2,y2]
		const res = this.baseResolution.scalar(0.5);
		return [
			this.transform.position.x - res.x,
			this.transform.position.y - res.y,
			this.transform.position.x + res.x,
			this.transform.position.y + res.y
		];
	}

	screenToWorldSpace2D(clientX, clientY){
		//takes in a  raw screen coordinate and converts it to world space

		let pos = new Vector2(clientX, clientY);
		const screenCenter = new Vector2(window.innerWidth/2, window.innerHeight/2);					//center of the screen
		const sCorner = screenCenter.subtract(this.viewPort.scalar(0.5));								//upper left corner of the viewPort in screen position
		const vCorner = this.transform.position.subtract(this.baseResolution.scalar(0.5)) 				//uppoer left corner of view in world position
		const tVector = vCorner.subtract(sCorner.scalar(new Vector2(1/this.scale.x, 1/this.scale.y))); 	//vector point from screen corner to view corner scaled
		let tMatrix = Matrix.identityMatrix(3,3);														//matrix for transofrming from screen space to world space
		tMatrix.setVal(0,2, tVector.x);
		tMatrix.setVal(1,2, tVector.y);
		tMatrix.setVal(0,0, 1/this.scale.x);
		tMatrix.setVal(1,1, 1/this.scale.y);
		tMatrix.setVal(2,2, 1/this.scale.x);

		let pMatrix = new Matrix(3,1);
		pMatrix.setVal(0,0, pos.x);
		pMatrix.setVal(1,0, pos.y);
		pMatrix.setVal(2,0,1);

		tMatrix.multiply(pMatrix);

		return new Vector2(tMatrix.data[0][0], tMatrix.data[1][0]);
